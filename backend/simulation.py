...
def simulate_power(sunlight, wind, hydro):
    """
    sunlight: 0–100 (%)
    wind: 0–25 (m/s) or 0–100 (%) — auto-detected
    hydro: 0–100 (%)
    """
    ...
    # --- Wind --- cubic relation; support both m/s and percent for backward compat
    # ponytail: branch handles legacy 0-100 percent callers; wind<=25 is m/s path
    if wind <= 25:
        rated = 12.0
        wind_norm = min(wind, rated) / rated
